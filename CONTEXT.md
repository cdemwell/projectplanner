...
- Bindings: `n` new, `u` update/edit, `m` move, `c` comment, `t` task, `x`
  task toggle/edit, `f` filter, `/` search, `e` toggle complete, `d` delete,
  `r` refresh, `q` quit. `e` toggles between a `done` workflow state and
  ...
  `completed_at` stays consistent). `x` opens `TaskActionScreen` (toggle a task's
  completion or edit its description).
